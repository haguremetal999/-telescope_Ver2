...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   int NN = 2;
   int PixelNumber = 128;
   double PixelSize = 36.0;
   double binwidth = 1;
   
   //////////////////////////////////////////
   MakeHist b;
   b.UnitFlag = true;
   b.SetRange1D(1024*binwidth,0,1024);   
   //   b.SetRange1D(100*binwidth,-50,50);
   //   b.SetImgNum(2000);
   b.SetRange2D(128,-64,64);
   b.SetDifRange(1000*binwidth,5);
   b.SetClusterRange(30,30);
   b.SetDUTRange(128,-64,64);
   b.SetPixelSize(PixelSize);
   b.SetHX1D();
   b.SetHY1D();
   b.SetH2D();
   b.SetReconstructionH2D();
   b.SetDifH1D();
   b.SetHCluster1D();
   b.SetEnergyH1D();
   b.SetFirstH2D();
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	continue;
      ...
      //first position histgram making [mm]
       if(debug2) std::cout << "j=" << jentry << " : (x[mm],y[mm) = ( " << BeamPosX0 <<", "<< BeamPosY0 << ")" << std::endl;
   ...
   //---------------------------------------IADR & Energyloss setting --------------------------------------
       ...
       //////////////////////////////////////////       
       LoopAnalysis a(jentry);
       a.SetUp(IADRl, energylossl,Nhits,PixelSize,PixelNumber);
       if(a.GetLastHit()[3]==0){
	 std::cout << "==================No last hit!" << std::endl;
	 continue;
       }
       //       if(a.GetID[0])
       if(a.GetID()[0]!=0){
	 std::cout<<  "No first hit! --ID[0] = " << a.GetID()[0] << std::endl;
	 continue;
       }
       a.SetThreshold(0.1,1);
       a.SetPitch(30.0*1000);
       a.SetHitPixel();
       a.MakeCluster(NN);
       a.SetReconstruction();
       //////////////////////////////////////////
     ...
     //////////////////////////////////////////
     b.PushDataHX1D(a.GetMeanX());
     b.PushDataHY1D(a.GetMeanY());
     b.PushDataH2D(a.GetMeanX(),a.GetMeanY());
     b.PushDataReconstructionH2D(a.GetReX(),a.GetReY());
     b.PushDataDifH1D(a.GetDifX(),a.GetDifY());
     b.PushDataHCluster1D(a.GetClusterSize());
     b.PushDataEnergyH1D(a.GetTotalLoss(),a.GetLoss(),Nhits);
     b.PushDataFirstH2D(BeamPosX0*1000/PixelSize+PixelNumber/2, BeamPosY0*1000/PixelSize+PixelNumber/2);
     //////////////////////////////////////////
     
     for(int nn=0;nn<4;nn++){
       if(debug3){
	 std::cout << "================================================" << std::endl;
	 std::cout << "meanx ["<< jentry << "][" << nn  << "]= "  << a.GetMeanX()[nn] << std::endl;
	 std::cout << "meany ["<< jentry << "][" << nn  << "]= "  << a.GetMeanY()[nn] << std::endl;
       }
     }
     if(debug5){
       std::cout << "================================================" << std::endl;
       std::cout << "rex  ["<< jentry << "] = "  << a.GetReX()  << std::endl;
       std::cout << "rey  ["<< jentry << "] = "  << a.GetReY()  << std::endl;
       std::cout << "difx ["<< jentry << "] = "  << a.GetDifX() << std::endl;
       std::cout << "dify ["<< jentry << "] = "  << a.GetDifY() << std::endl;
     }
   } //jentry
   ...
   if(!ofile){
     ...
     ofile << b.GetXspread() << std::endl;
     ofile << b.GetYspread() << std::endl;
     ofile << b.GetXdif() << std::endl;
     ofile << b.GetYdif();
