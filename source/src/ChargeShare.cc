...
void ChargeShare::setPositionYx(G4double yp, G4double xp){
  ...
  if( (xpos<-wx*1.5 || xpos>wx*1.5 || ypos<-wy*1.5 || ypos>wy*1.5)) {  //defalt setting
...
ChargeShare::ChargeShare (
       G4double width_y,   // pixel size in Y 
       G4double width_x,   // pixel size in X 
  ...
  sig=sigma;
#else
  //  wx=width_x/4;     //  wx and wy are quater pixel size // defalt setting
  //  wy=width_y/4;     //  wx and wy are quater pixel size // defalt setting
  //  sig=sigma/4;
  wx=width_x/2;
  wy=width_y/2;
  sig=sigma;
